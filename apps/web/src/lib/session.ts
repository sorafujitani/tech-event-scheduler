...
export const createSessionResolver =
  (deps: { authFetch: Fetcher; ssr: boolean }) =>
  async (): Promise<SessionContext | null> => {
    if (deps.ssr) {
      // M4: better-auth は /auth/* ワイルドカードで RPC アクセサに出ない。
      //     同一 fetch 経路(service binding / localhost + Cookie 転送)で /api/auth/get-session を素で叩く。
      const res = await deps.authFetch("/api/auth/get-session", {
        method: "GET",
      });
